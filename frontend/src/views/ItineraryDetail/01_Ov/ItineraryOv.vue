    ...
    <div v-if="tripPlan" class="content-wrapper">
      ...
      <div class="main-content">
        ...
        <div class="outer-container">
          ...
          <!-- 右侧：Tag 设置卡 -->
<div class="tag-setting-card">
  <div class="title-bar">
    <span class="card-title">Tag 设置</span>
    <span class="card-desc">回车或点击「添加」，标签会出现在下方</span>
  </div>
  ...
  <div class="tag-input-row">
    <a-input
      ...
      @keyup.enter="addTag"
    />
    <button class="edit-btn add-btn" @click="addTag">添加</button>
  </div>
  ...
  <div class="tag-list">
    ...
    <div v-if="tagList.length === 0" class="empty-hint">
      还没有标签，先添加一个 ↑
    </div>
  </div>
</div>
...
function addTag() {
  ...
  if (tagList.value.some(t => t.text === text)) {    // 去重
    newTag.value = ''
    return
  }
  tagList.value.push({ id: uid++, text })
  newTag.value = ''                                  // 清空，方便连续输入
}
...
.title-bar {
  padding: 180px 50px 100px 30px;
  /* 底部增加到 170px，为右下角按钮预留空间 */
  display: flex;
  flex-direction: column;
...
.add-btn:hover  { transform: translateY(-1px); box-shadow: 0 3px 8px rgba(0,0,0,.15); }
...
.added-tag {
  margin: 0;              /* 抵消 a-tag 默认右边距，间距统一交给 gap 管 */
  cursor: default;
  transition: transform .15s ease, box-shadow .15s ease;
}
.added-tag:hover { transform: translateY(-2px); box-shadow: 0 3px 8px rgba(0,0,0,.12); }
...
/* 标签入场 / 离场动画（配合 TransitionGroup） */
.tag-pop-enter-active { transition: all .25s cubic-bezier(.2, .9, .3, 1.3); }
.tag-pop-leave-active { transition: all .18s ease; position: absolute; }
.tag-pop-enter-from   { opacity: 0; transform: translateY(8px) scale(.85); }
.tag-pop-leave-to     { opacity: 0; transform: scale(.85); }
.tag-pop-move         { transition: transform .25s ease; }
